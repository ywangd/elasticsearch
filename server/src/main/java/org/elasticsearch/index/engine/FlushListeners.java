...
public class FlushListeners implements Closeable {
    ...
    public void addOrNotify(Translog.Location location, ActionListener<Long> listener) {
        logger.info("--> FlushListeners#addOrNotify [{}] [{}]", location, listener);
        requireNonNull(listener, "listener cannot be null");
        requireNonNull(location, "location cannot be null");

        Tuple<Long, Translog.Location> lastCommitBeforeSynchronized = lastCommit;
        logger.info("--> lastCommitBeforeSynchronized [{}]", lastCommitBeforeSynchronized);
        ...
        synchronized (this) {
            ...
            logger.info("--> lastCommitBeforeSynchronized inside sync [{}]", lastCommitAfterSynchronized);
            ...
            logger.info("--> adding flush listener [{}]", location);
    ...
    public void afterFlush(final long generation, final Translog.Location lastCommitLocation) {
        logger.info("--> afterFlush updating for [{}] [{}]", generation, lastCommitLocation);
        ...
        synchronized (this) {
            ...
            for (Tuple<Translog.Location, ActionListener<Long>> tuple : locationCommitListeners) {
                ...
                if (location.compareTo(lastCommitLocation) <= 0) {
                    ...
                    logger.info("--> adding flush listener to fire, waiting for [{}] <= [{}]", location, lastCommitLocation);
