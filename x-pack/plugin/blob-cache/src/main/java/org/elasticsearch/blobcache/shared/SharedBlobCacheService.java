...
public class SharedBlobCacheService<KeyType> implements Releasable {
    ...
    public class CacheFileRegion extends EvictableRefCounted {
        ...
        boolean tryEvict() {
            ...
            if (refCount() <= 1 && evict()) {
                ...
                if (shouldLog) {
                    logger.info("--> tryEvict: {}, [{}]", this, Thread.currentThread().getName());
                }
        ...
        public boolean forceEvict() {
            ...
            if (evict()) {
                ...
                if (shouldLog) {
                    logger.info("--> forceEvict: {}, [{}]", this, Thread.currentThread().getName());
                }
                evictCount.increment();
                decRef();
                return true;
        ...
        void populateAndRead(
            ...
            try {
                incRef();
                if (shouldLog) {
                    logger.info("--> populateAndRead: {}, [{}]", this, Thread.currentThread().getName());
                }
    ...
    private class LFUCache implements Cache<KeyType, CacheFileRegion> {
        ...
        private LFUCacheEntry initChunk(LFUCacheEntry entry) {
            ...
            if (shouldLog) {
                logger.info("--> initChunk: region=[{}], [{}]", entry.chunk, Thread.currentThread().getName());
            }
            return entry;
        }
