 ...
 * provided role.
 */
public final class AndRole extends Role {
    private final Role fromRole;
    private final Role limitedBy;

    AndRole(Role fromRole, Role limitedBy) {
        super(fromRole.names(), fromRole.cluster(), fromRole.indices(), fromRole.application(), fromRole.runAs());
    ...
    public IndicesAccessControl authorize(String action, Set<String> requestedIndicesOrAliases,
        ...
        IndicesAccessControl indicesAccessControl =
            fromRole.authorize(action, requestedIndicesOrAliases, aliasAndIndexLookup, fieldPermissionsCache);
    ...
    public Automaton allowedActionsMatcher(String index) {
        ...
        final Automaton limitedByMatcher = fromRole.allowedActionsMatcher(index);
    ...
    public boolean checkIndicesAction(String action) {
        return fromRole.checkIndicesAction(action) && limitedBy.checkIndicesAction(action);
    }
    ...
    public boolean checkClusterAction(String action, TransportRequest request, Authentication authentication) {
        return fromRole.checkClusterAction(action, request, authentication) && limitedBy.checkClusterAction(action, request, authentication);
    }
    ...
    public boolean grants(ClusterPrivilege clusterPrivilege) {
        return fromRole.grants(clusterPrivilege) && limitedBy.grants(clusterPrivilege);
    }
    ...
    public boolean checkRunAs(String runAs) {
        return fromRole.checkRunAs(runAs) && limitedBy.checkRunAs(runAs);
    }
     ...
     * @return {@link AndRole}
     */
    public static AndRole createAndRole(Role fromRole, Role limitedByRole) {
        Objects.requireNonNull(limitedByRole, "limited by role is required to create limited role");
        return new AndRole(fromRole, limitedByRole);
    }
}
