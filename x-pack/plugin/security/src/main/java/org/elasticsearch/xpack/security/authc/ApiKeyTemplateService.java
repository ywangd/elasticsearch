...
public class ApiKeyTemplateService {

    private static final Logger logger = LogManager.getLogger(ApiKeyTemplateService.class);
    ...
    public ApiKeyTemplateService(Settings settings, Clock clock, Client client, XPackLicenseState licenseState, SecurityIndexManager securityIndex,
    ...
    public void createApiKeyTemplate(Authentication authentication, CreateApiKeyTemplateRequest request, Set<RoleDescriptor> userRoles,
                             ActionListener<CreateApiKeyTemplateResponse> listener) {
        ensureEnabled();
        if (authentication == null) {
            listener.onFailure(new IllegalArgumentException("authentication must be provided"));
        } else {
            createApiKeyTemplateAndIndexIt(authentication, request, userRoles, listener);
        }
    }

    private void createApiKeyTemplateAndIndexIt(Authentication authentication, CreateApiKeyTemplateRequest request,
        Set<RoleDescriptor> roleDescriptorSet, ActionListener<CreateApiKeyTemplateResponse> listener) {
    ...
    XContentBuilder newTemplateDocument(String name, Authentication authentication, Set<RoleDescriptor> userRoles,
        ...
        final long creation_time = created.toEpochMilli();
        builder.startObject()
            .field("doc_type", "api_key_template")
            .field("creation_time", creation_time)
            .field("expiration_time", expiration == null ? null : expiration.toEpochMilli())
            .field("key_expiration", keyExpiration == null ? null : keyExpiration.getMillis())
            .field("api_key_template_invalidated", false);

        // Save role_descriptors
        builder.startObject("role_descriptors");
        if (templateRoles != null && templateRoles.isEmpty() == false) {
            for (RoleDescriptor descriptor : templateRoles) {
...
//            buildRoleDescriptorSet((Map<String, Object>) source.get("limited_by_role_descriptors"), request.isSyncable());
    ...
    private Instant getApiKeyTemplateExpiration(Instant now, CreateApiKeyTemplateRequest request) {
